...
async function loadConfig(){
 const r=await supabaseClient.from('site_config').select('*').eq('id',1).maybeSingle();
 SITE_CONFIG={...DEFAULT_CONFIG,...(r.data||{})};

 const nome=(SITE_CONFIG.nome_corretor||'Ricardo Almeida').trim();
 const creci=(SITE_CONFIG.creci||'CRECI 123456-F').trim();

 document.querySelectorAll('.brand strong').forEach(e=>e.textContent=nome.toUpperCase());
 document.querySelectorAll('.brand small').forEach(e=>e.textContent=`CORRETOR DE IMÓVEIS · ${creci}`);
 ...
 const parts=String(SITE_CONFIG.titulo_principal||'').split('|');
 const h1=document.querySelector('.hero h1');
 if(h1) h1.innerHTML=`${parts[0]||''}<span>${parts[1]||''}</span>`;

 const lead=document.querySelector('.hero .lead');
 if(lead) lead.textContent=SITE_CONFIG.subtitulo_principal||'';

 const bg=document.querySelector('.hero-bg');
 if(bg&&SITE_CONFIG.banner_url){
   bg.style.background=`radial-gradient(circle at 75% 25%,#d8a84f30,transparent 24%),linear-gradient(90deg,#07101b 0%,#07101bee 35%,#07101b44 70%),url('${SITE_CONFIG.banner_url}') center/cover no-repeat`;
 }

 const cp=document.querySelector('#contato p,.contact p');
 if(cp) cp.textContent=SITE_CONFIG.texto_contato||'';

 const footer=document.getElementById('footerText');
 if(footer){
   const salvo=String(SITE_CONFIG.footer_text||'').trim();
   const rodapeAutomatico=`© 2026 ${nome} · Corretor de Imóveis · ${creci}`;
   footer.textContent=(!salvo || salvo===LEGACY_DEFAULT_FOOTER) ? rodapeAutomatico : salvo;
 }
 ...
 if(agentCard){
   ...
   if(heroContent) heroContent.style.gridTemplateColumns=showAgent?'1.05fr .95fr':'1fr';
 }
 if(agentPhoto&&SITE_CONFIG.agent_url){
   agentPhoto.style.background=`linear-gradient(180deg,#ffffff05,#0008),url('${SITE_CONFIG.agent_url}') center bottom/contain no-repeat`;
 }

 document.querySelectorAll('a[href*="wa.me"],.whatsapp').forEach(a=>{if(a.tagName==='A')a.href=`https://wa.me/${nums(SITE_CONFIG.whatsapp)}`});
 return SITE_CONFIG;
}
...
function setupContact(cfg){
  ...
  form.addEventListener('submit',async function(e){
    ...
    if(!nome||!whatsapp){alert('Por favor, preencha seu nome e WhatsApp.');return false;}

    try{
      const {error}=await supabaseClient.from('mensagens_contato').insert({nome,whatsapp,interesse,mensagem});
      if(error) console.warn('Erro ao salvar no Supabase:',error.message);
    }catch(err){console.warn('Não foi possível salvar a mensagem no Supabase:',err);}

    let box=document.getElementById('contact-success');
    if(!box){box=document.createElement('div');box.id='contact-success';form.appendChild(box);}
    box.style.cssText='margin-top:15px;padding:12px 16px;background:#0b1119;border:1px solid #d8a84f55;border-radius:8px;color:#d8a84f;font-weight:800;line-height:1.5';
    box.textContent='Sua mensagem foi enviada para o corretor. Em breve retornaremos em seu WhatsApp!';
...
Mensagem: ${mensagem}`;
    ...
    if(numeroCorretor){window.open(`https://wa.me/${numeroCorretor}?text=${encodeURIComponent(texto)}`,'_blank','noopener,noreferrer');}
    else{console.warn('Número do corretor não configurado.');}
